...
const WELCOME_MSG = [
  ...
  '\x1b[purple]  ╚══════╝  ╚═════╝  ╚═════╝  ╚═╝  ╚═╝ ╚═╝  ╚═╝ ╚═╝     ╚═╝  ╚═════╝  ╚══════╝\x1b[/]',
  '',
  '  \x1b[cyan]Welcome to SubhamOS Terminal v2.0\x1b[/]',
...
const NEOFETCH = [
  '',
  '\x1b[purple]        ▄▄▄▄▄▄▄▄▄▄▄▄▄       \x1b[/] \x1b[cyan]subham\x1b[/]\x1b[white]@\x1b[/]\x1b[purple]portfolio\x1b[/]',
  '\x1b[purple]      ▄█████████████████▄     \x1b[/] \x1b[white]─────────────────\x1b[/]',
  '\x1b[purple]    ▄███████████████████████▄   \x1b[/]\x1b[cyan]OS:\x1b[/]       SubhamOS v2.0',
  ...
  '\x1b[purple]  ██████████████████████████████ \x1b[/]\x1b[cyan]DE:\x1b[/]       SubhamOS Desktop',
...
const HELP_TEXT = [
  '',
  '  \x1b[yellow]╔══════════════════════════════════════════════════════════╗\x1b[/]',
  '  \x1b[yellow]║\x1b[/]          \x1b[cyan]SubhamOS Terminal — Command Reference\x1b[/]          \x1b[yellow]║\x1b[/]',
  '  \x1b[yellow]╠══════════════════════════════════════════════════════════╣\x1b[/]',
...
const WHOAMI_OUTPUT = [
  ...
  '  \x1b[cyan]│\x1b[/]  \x1b[white]Shell:\x1b[/]     SubhamOS Terminal 2.0  \x1b[cyan]│\x1b[/]',
  '  \x1b[cyan]└────────────────────────────────────┘\x1b[/]',
  '',
];
...
function TerminalLine({ parts }) {
  return (
    <div className="whitespace-pre leading-[1.55] font-mono" style={{ fontSize: '12.5px' }}>
      {parts.map((part, i) => (
        <span key={i} style={part.color ? { color: part.color } : { color: '#94a3b8' }}>
          {part.text}
        </span>
      ))}
    </div>
...
const TerminalApp = ({ onOpenApp }) => {
  const [lines, setLines] = useState(() => WELCOME_MSG.map((l) => ({ type: 'output', parts: parseLine(l) })));
  ...
  const addPromptLine = (cmd) => {
    setLines((prev) => [
      ...prev,
      {
        type: 'prompt',
        parts: [
          { text: 'subham', color: colorMap.green },
  ...
  const processCommand = async (rawCmd) => {
    ...
    switch (cmd) {
      ...
      case 'date':
        addOutput([
          '',
          `  \x1b[cyan]${new Date().toString()}\x1b[/]`,
          '',
        ]);
        break;
      ...
      case 'history':
        if (commandHistory.length === 0) {
          ...
          addOutput([
            ...
            '',
            ...commandHistory.map((c, i) => `  \x1b[gray]${String(i + 1).padStart(4, ' ')}\x1b[/]  ${c}`),
            '',
          ]);
        }
        break;
      ...
      case 'cd':
        addOutput([
          '',
          `  \x1b[gray]Navigated to ${args || '~'}\x1b[/]`,
          '',
        ]);
        break;
      ...
      case 'cat':
        if (args.toLowerCase().includes('readme')) {
          addOutput([
            '',
            '  \x1b[yellow]# SubhamOS Portfolio\x1b[/]',
            '',
          ...
          addOutput([`  \x1b[red]cat: ${args || '?'}: No such file or directory\x1b[/]`]);
        }
        break;
      ...
      case 'sudo':
        if (trimmed.includes('rm -rf')) {
          ...
          addOutput(['  \x1b[red]  Deleting /home/subham/skills... ████████░░ 80%\x1b[/]']);
          await new Promise((r) => setTimeout(r, 600));
          addOutput(['  \x1b[red]  Deleting /home/subham/projects... ██████░░░░ 60%\x1b[/]']);
          await new Promise((r) => setTimeout(r, 600));
          addOutput(['  \x1b[red]  Deleting /home/subham/life... ████░░░░░░ 40%\x1b[/]']);
          await new Promise((r) => setTimeout(r, 800));
          addOutput([
            '',
          ...
          addOutput([
            '',
            `  \x1b[red][sudo] password for subham: Nice try! 🔒\x1b[/]`,
            '',
          ]);
        }
        break;
      ...
      case 'ping':
        addOutput([
          '',
          `  \x1b[cyan]PING ${args || 'subham.dev'} — 64 bytes: time=0.42ms TTL=64\x1b[/]`,
          `  \x1b[cyan]PING ${args || 'subham.dev'} — 64 bytes: time=0.38ms TTL=64\x1b[/]`,
          `  \x1b[cyan]PING ${args || 'subham.dev'} — 64 bytes: time=0.41ms TTL=64\x1b[/]`,
          '',
          `  \x1b[gray]--- ${args || 'subham.dev'} ping statistics ---\x1b[/]`,
      ...
      case 'cowsay':
        const msg = args || 'Moo! Hire Subham!';
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === 'Enter') {
      ...
      const commands = [
        'help', 'neofetch', 'whoami', 'about', 'skills', 'projects',
        'social', 'leetcode', 'github', 'contact', 'run', 'resume',
        'date', 'echo', 'clear', 'banner', 'history', 'ls', 'pwd',
        'cd', 'cat', 'sudo', 'exit', 'matrix', 'ping', 'cowsay', 'fortune',
      ];
    ...
    >
      ...
      <div className="flex items-center gap-2 px-4 py-1.5 bg-[#0d1117] border-b border-white/5 shrink-0">
        <div className="flex items-center gap-1.5 text-[10px] text-white/20">
          <span className="px-1.5 py-0.5 bg-white/5 rounded text-green-400/60">bash</span>
          <span>—</span>
          <span>subham@portfolio:~</span>
        </div>
      </div>
      ...
      >
        ...
        {/* Current input line */}
        {!isProcessing && (
          <div className="flex items-center whitespace-pre font-mono leading-[1.55]" style={{ fontSize: '12.5px' }}>
            <span style={{ color: colorMap.green }}>subham</span>
